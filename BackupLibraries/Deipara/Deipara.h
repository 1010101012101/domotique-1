...
const unsigned long SALON_ADDR=0x408CCB42;
...
void sendZigBeeMsg(XBee iXbee, unsigned int iPayLoad, unsigned long iAddrToTarget)
{
  ...
  if (iXbee.readPacket(5000)) {
    //Serial.println("We got a response to the message");

    // should be a znet tx status  
    ZBTxStatusResponse aZbTxStatus = ZBTxStatusResponse();        
    if (iXbee.getResponse().getApiId() == ZB_TX_STATUS_RESPONSE) {
      iXbee.getResponse().getZBTxStatusResponse(aZbTxStatus);
      ...
      Serial.println(iXbee.getResponse().getApiId());
    }   
  } 
  else {
...
void sendZigBeeMsg2(XBee iXbee,int iCommand,int iDataToSend, unsigned long iAddrToTarget)
{
  //Serial.println("We are going to send a ZigBee message");
  // Create an array for holding the data you want to send.
  uint8_t aPayload[3];
